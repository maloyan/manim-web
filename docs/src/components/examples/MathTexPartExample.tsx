...
async function animate(scene: any) {
  ...
  await scene.play(
    new Indicate(equation.getPart(0), { color: YELLOW, duration: 0.6 }),
  );
  await scene.play(
    new Indicate(equation.getPart(2), { color: YELLOW, duration: 0.6 }),
  );
  await scene.play(
    new Indicate(equation.getPart(4), { color: YELLOW, duration: 0.6 }),
  );
  await scene.wait(1);
}
