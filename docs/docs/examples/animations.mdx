...
import BroadcastExample from '@site/src/components/examples/BroadcastExample';
...
import ApplyPointwiseFunctionToCenterExample from '@site/src/components/examples/ApplyPointwiseFunctionToCenterExample';
import LabeledPolygramExample from '@site/src/components/examples/LabeledPolygramExample';
...
<ApplyPointwiseFunctionToCenterExample />
...
**Learn More:** [**ApplyPointwiseFunctionToCenter**](/api/classes/ApplyPointwiseFunctionToCenter) · [**Circle**](/api/classes/Circle) · [**Square**](/api/classes/Square)
...
**Learn More:** [**Broadcast**](/api/classes/Broadcast) · [**Circle**](/api/classes/Circle) · [**Create**](/api/classes/Create) · [**FadeIn**](/api/classes/FadeIn)
...
**Learn More:** [**LabeledPolygram**](/api/classes/LabeledPolygram) · [**Polygram**](/api/classes/Polygram) · [**Polygon**](/api/classes/Polygon) · [**FadeIn**](/api/classes/FadeIn)

---

## Multi-Camera Composition

The next three examples build on `Scene.useMultiCamera()` to render the same
2D scene through several `Camera2D` viewports at once. Two ergonomics knobs
worth knowing about:

- `aspectMode: 'contain'` on each `Camera2D` keeps the per-camera
  `frameWidth` / `frameHeight` honoured at render time. Without it,
  `MultiCamera` would stretch each camera's width to match its
  viewport aspect and the zoom intent would be silently lost.
- `MultiCamera.setViewportBorder(index, { borderColor, borderWidth })`
  draws a pixel-space outline around a viewport so the camera regions
  are visible to the viewer.

### Split Screen Camera

Renders the same scene through two `Camera2D` instances side by side via
`SplitScreenCamera`. The left pane shows the wide world; the right pane is
tightly zoomed on the square. Each viewport gets a thin grey border so the
split is visible against the dark background.
...
Main camera shows the full scene while a small picture-in-picture inset
follows an orbiting dot at higher zoom. A yellow border highlights the
inset so the PiP region is unmissable.
...
Four simultaneous viewports of the same scene — one overview plus three
close-ups — composed with `MultiCamera.setupQuadView()`. Each pane gets
its own border tint so the four-cell grid is unmistakable.
