...
```typescript
import {
  BLUE, DOWN, Difference, Ellipse, Exclusion, FadeIn, GREEN, Group,
  Intersection, LEFT, MoveToTarget, ORANGE, PINK, RED, RIGHT, Scene,
  Text, Underline, UP, Union, WHITE, YELLOW, scaleVec
} from 'manim-js';

const scene = new Scene(document.getElementById('container'), {
  width: 800, height: 450, backgroundColor: '#000000'
});

const ellipse1 = new Ellipse({
  width: 4.0, height: 5.0, fillOpacity: 0.5, color: BLUE, strokeWidth: 2
}).moveTo(LEFT);
const ellipse2 = ellipse1.copy().setColor(RED).moveTo(RIGHT);
const bool_ops_text = new Text({
  text: 'Boolean Operation', fontFamily: 'serif', fontSize: 48
}).nextTo(ellipse1, UP);
...
u.generateTarget();
u.targetCopy.scale(0.3).setStrokeWidth(1.2).nextTo(i, DOWN, union_text.getHeight() * 3);
...
e.generateTarget();
e.targetCopy.scale(0.3).setStrokeWidth(1.2).nextTo(u, DOWN, exclusion_text.getHeight() * 3.5);
...
d.generateTarget();
d.targetCopy.scale(0.3).setStrokeWidth(1.2).nextTo(u, LEFT, difference_text.getHeight() * 3.5);
