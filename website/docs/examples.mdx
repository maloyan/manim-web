...
async function braceAnnotation(scene: Scene) {
  ...
  const b2 = new Brace(line, { direction: line.copy().rotate(Math.PI / 2).getUnitVector() });
...
Demonstrates a zoomed camera scene with pop-out display, frame animations, and scale/shift operations. Creates a grid of colored dots, sets up a zoomed camera frame, animates the display popping out, then scales and shifts both frame and display independently.
...
**Learn More:** **ZoomedScene** · **BackgroundRectangle** · **Scale** · **ScaleInPlace** · **UpdateFromFunc**
