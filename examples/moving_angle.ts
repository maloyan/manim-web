...
async function movingAngle(scene) {
  ...
  line_moving.rotate(theta_tracker.getValue() * (Math.PI / 180), {
    aboutPoint: rotation_center,
  });
  const a = new Angle({ line1: line1, line2: line_moving }, {
    radius: 0.5,
    otherAngle: false,
  });
  ...
  line_moving.addUpdater((x) => {
    ...
    x.rotate(theta_tracker.getValue() * (Math.PI / 180), {
      aboutPoint: rotation_center,
    });
  });
  ...
  a.addUpdater((x) =>
    x.become(
      new Angle({ line1: line1, line2: line_moving }, {
        radius: 0.5,
        otherAngle: false,
      }),
    )
