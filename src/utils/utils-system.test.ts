...
describe("logger - sanitization of structured data", () => {
  ...
  it("sanitizes nested objects recursively", () => {
    ...
    const logged = infoSpy.mock.calls[0][1] as Record<
      string,
  ...
  it("redacts multiple sensitive patterns in a single string", () => {
    ...
    expect((logged.match(/\[REDACTED\]/g) || []).length).toBeGreaterThanOrEqual(
      2,
    );
  });
  ...
  it("handles mixed argument types in a single call", () => {
    logger.info("prefix", {
      key: "ghp_" + "abc123456789012345678901234567890123",
    }, 42);
    expect(infoSpy.mock.calls[0][1]).toBe("prefix");
    expect(String((infoSpy.mock.calls[0][2] as Record<string, unknown>).key))
      .toContain(
        "[REDACTED]",
      );
...
describe("saveMobjectState / restoreMobjectState", () => {
  it("captures state and stores it on the mobject", () => {
    ...
    expect((vm as unknown as Record<string, unknown>).__savedMobjectState).toBe(
      state,
    );
...
describe("snapshotToJSON / snapshotFromJSON", () => {
  ...
  it("handles snapshot with no label", () => {
    const parsed = snapshotFromJSON(
      snapshotToJSON({ timestamp: 123, mobjects: [] }),
    );
