...
import { DOWN, LEFT, RIGHT, UP } from "../../core/Mobject";
...
describe("VMobjectFromSVGPath", () => {
  ...
  it("uses default color WHITE and custom color", () => {
    expect(new VMobjectFromSVGPath({ pathData: "M 0,0 L 10,0" }).color).toBe(
      WHITE,
    );
    expect(
      new VMobjectFromSVGPath({ pathData: "M 0,0 L 10,0", color: "#ff0000" })
        .color,
    ).toBe(
      "#ff0000",
    );
  ...
  it("handles H, V, Z commands", () => {
    expect(new VMobjectFromSVGPath({ pathData: "M 0,0 H 10" }).numPoints)
      .toBeGreaterThan(0);
    expect(new VMobjectFromSVGPath({ pathData: "M 0,0 V 10" }).numPoints)
      .toBeGreaterThan(0);
    expect(
  ...
  it("handles relative commands (l, m, h, v, c, s, q, t)", () => {
    ...
    expect(new VMobjectFromSVGPath({ pathData: "m 5,5 l 10,0" }).numPoints)
      .toBeGreaterThan(0);
    expect(new VMobjectFromSVGPath({ pathData: "M 0,0 h 10" }).numPoints)
      .toBeGreaterThan(0);
    expect(new VMobjectFromSVGPath({ pathData: "M 0,0 v 10" }).numPoints)
      .toBeGreaterThan(0);
    expect(
      new VMobjectFromSVGPath({ pathData: "M 0,0 c 5,10 15,10 20,0" })
        .numPoints,
    ).toBeGreaterThan(0);
    expect(
      new VMobjectFromSVGPath({
        pathData: "M 0,0 c 5,10 15,10 20,0 s 15,-10 20,0",
      }).numPoints,
    ).toBeGreaterThan(0);
    expect(new VMobjectFromSVGPath({ pathData: "M 0,0 q 5,10 10,0" }).numPoints)
      .toBeGreaterThan(0);
    expect(
      new VMobjectFromSVGPath({ pathData: "M 0,0 q 5,10 10,0 t 10,0" })
        .numPoints,
    ).toBeGreaterThan(0);
  });
  ...
  it("handles S command with and without preceding C", () => {
    ...
    expect(new VMobjectFromSVGPath({ pathData: "M 0,0 S 5,10 10,0" }).numPoints)
      .toBeGreaterThan(0);
  });

  it("handles T command with and without preceding Q", () => {
    expect(
      new VMobjectFromSVGPath({ pathData: "M 0,0 Q 5,10 10,0 T 20,0" })
        .numPoints,
    ).toBeGreaterThan(0);
    // T without preceding Q
    expect(new VMobjectFromSVGPath({ pathData: "M 0,0 T 10,0" }).numPoints)
      .toBeGreaterThan(0);
  });

  it("handles arc A command", () => {
    expect(
      new VMobjectFromSVGPath({ pathData: "M 10,80 A 25,25 0 0,1 50,80" })
        .numPoints,
    ).toBeGreaterThan(0);
  });
  ...
  it("handles arc with large-arc and sweep flags", () => {
    expect(
      new VMobjectFromSVGPath({ pathData: "M 10,80 A 25,25 0 1,0 50,80" })
        .numPoints,
    ).toBeGreaterThan(0);
    expect(
      new VMobjectFromSVGPath({ pathData: "M 10,80 A 25,25 0 1,1 50,80" })
        .numPoints,
    ).toBeGreaterThan(0);
  });
  ...
  it("handles degenerate arc (rx=0)", () => {
    expect(
      new VMobjectFromSVGPath({ pathData: "M 0,0 A 0,25 0 0,1 50,0" })
        .numPoints,
    ).toBeGreaterThan(0);
  });
  ...
  it("handles relative arc command", () => {
    expect(
      new VMobjectFromSVGPath({ pathData: "M 10,80 a 25,25 0 0,1 40,0" })
        .numPoints,
    ).toBeGreaterThan(0);
  });
  ...
  it("handles arc with rotation", () => {
    expect(
      new VMobjectFromSVGPath({ pathData: "M 10,80 A 25,50 45 0,1 50,80" })
        .numPoints,
    ).toBeGreaterThan(0);
  });
  ...
  it("handles arc where radii need scaling (too small)", () => {
    expect(
      new VMobjectFromSVGPath({ pathData: "M 0,0 A 1,1 0 0,1 100,100" })
        .numPoints,
    ).toBeGreaterThan(0);
  });
  ...
  it("handles multiple subpaths (M after first path)", () => {
    ...
    const path = new VMobjectFromSVGPath({
      pathData: "M 0,0 L 10,0 M 20,0 L 30,0",
    });
    expect(path.numPoints).toBeGreaterThan(0);
  });
});
...
describe("SVGMobject", () => {
  ...
  it("skips polygon with insufficient points", () => {
    const svg = new SVGMobject({
      svgString:
        '<svg xmlns="http://www.w3.org/2000/svg"><polygon points="50"/></svg>',
    });
    expect(svg.children.length).toBe(0);
  });
  ...
  it("skips polyline with insufficient points", () => {
    const svg = new SVGMobject({
      svgString:
        '<svg xmlns="http://www.w3.org/2000/svg"><polyline points="50"/></svg>',
    });
    expect(svg.children.length).toBe(0);
  });
  ...
  it("skips polyline and polygon without points attr", () => {
    const svg = new SVGMobject({
      svgString:
        '<svg xmlns="http://www.w3.org/2000/svg"><polygon/><polyline/></svg>',
    });
    expect(svg.children.length).toBe(0);
  });
...
describe("Brace", () => {
  ...
  it("getText accepts custom options", () => {
    const rect = new Rectangle({ width: 2, height: 1 });
    const brace = new Brace(rect, { direction: UP });
    const label = brace.getText("test", {
      fontSize: 24,
      color: "#ff0000",
      buff: 0.8,
    });
  ...
  it("getTex accepts custom options", () => {
    const rect = new Rectangle({ width: 2, height: 1 });
    const brace = new Brace(rect, { direction: UP });
    const label = brace.getTex("\\alpha", {
      fontSize: 24,
      color: "#ff0000",
      buff: 0.8,
    });
...
describe("ArcBrace", () => {
  ...
  it("accepts custom buff", () => {
    ...
    const distLarger = Math.sqrt(
      (largerTip[0] - center[0]) ** 2 + (largerTip[1] - center[1]) ** 2,
    );
