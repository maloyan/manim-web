...
describe("Circle", () => {
  ...
  it("getCircumference returns 2*PI*r", () => {
    expect(new Circle({ radius: 1 }).getCircumference()).toBeCloseTo(
      2 * Math.PI,
      10,
    );
    expect(new Circle({ radius: 5 }).getCircumference()).toBeCloseTo(
      10 * Math.PI,
      10,
    );
  });
...
describe("Arc", () => {
  ...
  it("getArcLength returns |r * angle|", () => {
    expect(new Arc({ radius: 2, angle: Math.PI }).getArcLength()).toBeCloseTo(
      2 * Math.PI,
      10,
    );
    expect(new Arc({ radius: 1, angle: Math.PI / 2 }).getArcLength())
