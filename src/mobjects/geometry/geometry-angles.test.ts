...
import { Angle, Elbow, RightAngle, TangentLine } from "./AngleShapes";
...
describe("Angle", () => {
  ...
  it("getLabel returns value with degree symbol when unit is degrees", () => {
    const label = new Angle({ points: PTS_90 }, {
      showValue: true,
      unit: "degrees",
    }).getLabel()!;
  ...
  it("quadrant 2: positive adjusts, negative passes through", () => {
    expect(
      new Angle({ points: PTS_90 }, { quadrant: 2 }).getLocalPoints().length,
    ).toBeGreaterThan(
      0,
    );
    expect(
      new Angle({ points: PTS_90_REV }, { quadrant: 2 }).getLocalPoints()
        .length,
    ).toBeGreaterThan(0);
  });
  ...
  it("quadrant 3: positive adjusts, negative passes through", () => {
    expect(
      new Angle({ points: PTS_90 }, { quadrant: 3 }).getLocalPoints().length,
    ).toBeGreaterThan(
      0,
    );
    expect(
      new Angle({ points: PTS_90_REV }, { quadrant: 3 }).getLocalPoints()
        .length,
    ).toBeGreaterThan(0);
  });
  ...
  it("quadrant 4: negative adjusts, positive passes through", () => {
    expect(
      new Angle({ points: PTS_90_REV }, { quadrant: 4 }).getLocalPoints()
        .length,
    ).toBeGreaterThan(0);
    expect(
      new Angle({ points: PTS_90 }, { quadrant: 4 }).getLocalPoints().length,
    ).toBeGreaterThan(
      0,
    );
  });
  ...
  it("axis option pins rotation plane so dir2 sweep stays continuous", () => {
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      const a = new Angle({ points: [dir1, [0, 0, 0], dir2] }, {
        radius: 1,
        axis: axisVec,
      });
    ...
    for (let i = 0; i < measured.length; i++) {
      if (i === measured.length - 1) {
        ...
        expect(
          Math.min(Math.abs(measured[i] - expected[i]), Math.abs(measured[i])),
        ).toBeLessThan(
          1e-3,
        );
      } else {
  ...
  it("negated axis reverses rotation direction", () => {
    ...
    const aPos = new Angle({ points: [dir1, [0, 0, 0], dir2] }, {
      axis: [0, 1, 0],
    });
    const aNeg = new Angle({ points: [dir1, [0, 0, 0], dir2] }, {
      axis: [0, -1, 0],
    });
...
describe("RightAngle", () => {
  ...
  it("constructs from two lines", () => {
    const l1 = new Line({ start: [0, 0, 0], end: [2, 0, 0] });
    const l2 = new Line({ start: [0, 0, 0], end: [0, 2, 0] });
    expect(new RightAngle({ line1: l1, line2: l2 }).getLocalPoints().length)
      .toBeGreaterThan(0);
  });
  ...
  it("custom size is applied", () => {
    expect(new RightAngle({ points: PTS_90 }, { size: 0.6 }).getSize()).toBe(
      0.6,
    );
  });
  ...
  it("line vertex detection: d2 min", () => {
    const l1 = new Line({ start: [0, 0, 0], end: [2, 0, 0] });
    const l2 = new Line({ start: [0, 2, 0], end: [0, 0, 0] });
    expect(new RightAngle({ line1: l1, line2: l2 }).getVertex()).toEqual([
      0,
      0,
      0,
    ]);
...
describe("TangentLine", () => {
  ...
  it("getTangentPoint is on the curve", () => {
    const pt = new TangentLine(new Circle({ radius: 2 }), { t: 0 })
      .getTangentPoint();
  ...
  it("tangent direction is a unit vector", () => {
    ...
    expect(Math.sqrt(dir[0] ** 2 + dir[1] ** 2 + dir[2] ** 2)).toBeCloseTo(
      1,
      5,
    );
  });
  ...
  it("getStart and getEnd are length apart", () => {
    ...
    const e = t.getEnd();
    expect(
      Math.sqrt((e[0] - s[0]) ** 2 + (e[1] - s[1]) ** 2 + (e[2] - s[2]) ** 2),
    ).toBeCloseTo(
      3,
      3,
    );
  ...
  it("handles VMobject with 2-3 points at t=1 (end boundary)", () => {
    ...
    expect(new TangentLine(vm, { t: 1 }).getTangentPoint()[0]).toBeCloseTo(
      2,
      1,
    );
  });
  ...
  it("falls back to default direction when derivative is zero", () => {
    ...
    expect(new TangentLine(vm, { t: 0.5 }).getTangentDirection()).toEqual([
      1,
      0,
      0,
    ]);
