...
describe("Text", () => {
  ...
  describe("constructor", () => {
    it("should create text with default options", () => {
      ...
      expect(t.getFontFamily()).toBe(
        "CMU Serif, Georgia, Times New Roman, serif",
      );
    });
    ...
    it("should store fontUrl when provided", () => {
      const t = new Text({
        text: "Custom",
        fontUrl: "http://example.com/font.otf",
      });
      expect((t as unknown as TextInternal)._fontUrl).toBe(
        "http://example.com/font.otf",
      );
    });
  });
  ...
  describe("_buildFontString", () => {
    it("should produce correct font string with defaults", () => {
      ...
      expect(fs).toBe(
        "normal normal 96pt CMU Serif, Georgia, Times New Roman, serif",
      );
    });
  ...
  describe("Three.js integration", () => {
    ...
    it("loadGlyphs awaits document.fonts.load to flush canvas font cache (issue #309)", async () => {
      ...
      const FakeFontFace = vi.fn(
        function (this: typeof fakeFace, family: string) {
      ...
      try {
        ...
        const url =
          `http://example.com/font-${Date.now()}-${Math.random()}.ttf`;
        ...
        expect(calls.indexOf("fonts.add")).toBeGreaterThan(
          calls.indexOf("face.load"),
        );
        const loadCallIdx = calls.findIndex((c) => c.startsWith("fonts.load:"));
        expect(loadCallIdx).toBeGreaterThan(calls.indexOf("fonts.add"));
        expect(fakeFonts.load).toHaveBeenCalledWith(
          expect.stringMatching(/^1em '/),
        );
      } finally {
  ...
  describe("dispose with mesh resources", () => {
    it("should dispose texture, geometry, and material when mesh exists", () => {
      ...
      const matDispose = vi.spyOn(
        internal._mesh!.material as THREE.Material,
        "dispose",
      );
      t.dispose();
  ...
  describe("_drawTextWithLetterSpacing null guard", () => {
    it("should return early if ctx is null", () => {
      ...
      internal._ctx = null;
      expect(() => internal._drawTextWithLetterSpacing("Test", 0, 0, 48)).not
        .toThrow();
    });
  });
  ...
  describe("getDisplayMeshes", () => {
    ...
    it("should throw when getThreeObject does not return a group", () => {
      ...
      expect(() => t.getDisplayMeshes()).toThrow(
        "Text.getThreeObject() must return a THREE.Group",
      );
    });
  });
