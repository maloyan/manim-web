...
function tokenTexts(code: Code, type: TokenType): string[] {
  ...
  for (let i = 1; i <= code.getLineCount(); i++) {
    ...
    if (line) {
      for (const t of line.tokens) if (t.type === type) result.push(t.text);
    }
  }
  return result;
}
...
describe("Code", () => {
  describe("constructor", () => {
    ...
    it("should accept various languages", () => {
      expect(new Code({ code: "x", language: "javascript" }).getLanguage())
        .toBe("javascript");
      expect(new Code({ code: "x", language: "typescript" }).getLanguage())
        .toBe("typescript");
    });
  ...
  describe("getLineOfCode", () => {
    ...
    it("should expand tabs in returned text", () => {
      expect(new Code({ code: "\thello", tabWidth: 4 }).getLineOfCode(1)!.text)
        .toBe("    hello");
      expect(new Code({ code: "\thello", tabWidth: 2 }).getLineOfCode(1)!.text)
        .toBe("  hello");
      expect(new Code({ code: "\t\tx", tabWidth: 4 }).getLineOfCode(1)!.text)
        .toBe("        x");
    });
  });
  ...
  describe("tokenization - Python", () => {
    ...
    it("should recognize self keyword", () => {
      expect(
        tokenTexts(
          new Code({ code: "self.x = 1", language: "python" }),
          "keyword",
        ),
      ).toContain(
        "self",
    ...
    it("should detect double and single-quoted strings", () => {
      const dq = tokenTexts(
        new Code({ code: 'x = "hello"', language: "python" }),
        "string",
      );
      expect(dq.length).toBeGreaterThan(0);
      expect(dq[0]).toContain("hello");
      const sq = tokenTexts(
        new Code({ code: "x = 'world'", language: "python" }),
        "string",
      );
    ...
    it("should detect function calls", () => {
      expect(
        tokenTexts(
          new Code({ code: 'print("hello")', language: "python" }),
          "function",
        ),
      ).toContain("print");
    });
    ...
    it("should detect integers, floats, and hex numbers", () => {
      expect(
        tokenTexts(new Code({ code: "x = 42", language: "python" }), "number"),
      ).toContain(
        "42",
      );
      expect(
        tokenTexts(
          new Code({ code: "x = 3.14", language: "python" }),
          "number",
        ),
      ).toContain(
        "3.14",
      ...
      expect(
        tokenTexts(
          new Code({ code: "x = 0xFF", language: "python" }),
          "number",
        ),
      ).toContain(
        "0xFF",
    ...
    it("should detect operators and punctuation", () => {
      expect(
        tokenTexts(
          new Code({ code: "x == y", language: "python" }),
          "operator",
        ),
      ).toContain(
        "==",
      ...
      const punc = tokenTexts(
        new Code({ code: "foo(x, y)", language: "python" }),
        "punctuation",
      );
    ...
    it("should detect builtin types", () => {
      expect(
        tokenTexts(
          new Code({ code: "x: int = 5", language: "python" }),
          "type",
        ),
      ).toContain(
        "int",
  ...
  describe("tokenization - JavaScript", () => {
    ...
    it("should detect // and /* */ comments", () => {
      const sl = tokenTexts(
        new Code({ code: "// comment", language: "javascript" }),
        "comment",
      );
    ...
    it("should detect strings, function calls, numbers, and operators", () => {
      expect(
        tokenTexts(
          new Code({ code: 'const s = "hello";', language: "javascript" }),
          "string",
        )
          .length,
      ).toBeGreaterThan(0);
      expect(
        tokenTexts(
          new Code({ code: 'console.log("hi")', language: "javascript" }),
          "function",
        ),
      ).toContain("log");
      expect(
        tokenTexts(
          new Code({ code: "const x = 42;", language: "javascript" }),
          "number",
        ),
      ).toContain("42");
  ...
  describe("tokenization - generic", () => {
    it("should handle empty and whitespace-only lines", () => {
      ...
      for (const t of ws.getLineOfCode(1)!.tokens) {
    ...
    it("should handle binary, octal, and scientific number literals", () => {
      expect(
        tokenTexts(
          new Code({ code: "x = 0b1010", language: "python" }),
          "number",
        ),
      ).toContain(
        "0b1010",
      ...
      expect(
        tokenTexts(
          new Code({ code: "x = 0o777", language: "python" }),
          "number",
        ),
      ).toContain(
        "0o777",
      ...
      expect(
        tokenTexts(
          new Code({ code: "x = 1e10", language: "python" }),
          "number",
        ),
      ).toContain(
        "1e10",
    ...
    it("should not treat identifier-embedded digits as numbers", () => {
      const nums = tokenTexts(
        new Code({ code: "x2 = 1", language: "python" }),
        "number",
      );
  ...
  describe("color schemes", () => {
    ...
    it("should have different colors between schemes", () => {
      expect(DEFAULT_COLOR_SCHEME.keyword).not.toBe(
        MONOKAI_COLOR_SCHEME.keyword,
      );
      expect(DEFAULT_COLOR_SCHEME.background).not.toBe(
        MONOKAI_COLOR_SCHEME.background,
      );
    });

    it("should have string values for all properties", () => {
      for (const v of Object.values(DEFAULT_COLOR_SCHEME)) {
        expect(typeof v).toBe("string");
      }
      for (const v of Object.values(MONOKAI_COLOR_SCHEME)) {
        expect(typeof v).toBe("string");
      }
    });
  });
  ...
  describe("tokenization - Python strings", () => {
    ...
    it("should handle unclosed string at end of line", () => {
      const strings = tokenTexts(
        new Code({ code: 'x = "unclosed', language: "python" }),
        "string",
      );
      expect(strings.length).toBeGreaterThan(0);
    });
  ...
  describe("tokenization - operators", () => {
    it("should detect arrow operator ->", () => {
      const ops = tokenTexts(
        new Code({ code: "fn -> int", language: "python" }),
        "operator",
      );
      expect(ops).toContain("->");
    });
    ...
    it("should detect comparison operators <=, >=", () => {
      const ops = tokenTexts(
        new Code({ code: "x <= y >= z", language: "python" }),
        "operator",
      );
      expect(ops).toContain("<=");
      expect(ops).toContain(">=");
    ...
    it("should detect shift operators <<, >>", () => {
      const ops = tokenTexts(
        new Code({ code: "x << 2 >> 1", language: "javascript" }),
        "operator",
      );
      expect(ops).toContain("<<");
      expect(ops).toContain(">>");
    ...
    it("should detect single character operators like ~, ^, |, &", () => {
      const ops = tokenTexts(
        new Code({ code: "~x ^ y | z & w", language: "python" }),
        "operator",
      );
      expect(ops).toContain("~");
      expect(ops).toContain("^");
  ...
  describe("tokenization - numbers", () => {
    it("should detect scientific notation with negative exponent", () => {
      const nums = tokenTexts(
        new Code({ code: "x = 1e-5", language: "python" }),
        "number",
      );
    ...
    it("should detect scientific notation with positive exponent", () => {
      const nums = tokenTexts(
        new Code({ code: "x = 2.5e+10", language: "python" }),
        "number",
      );
  ...
  describe("Rust string delimiters", () => {
    ...
    it("should handle Rust char literals", () => {
      const strings = tokenTexts(
        new Code({ code: "let c = 'a';", language: "rust" }),
        "string",
      );
      expect(strings.length).toBeGreaterThan(0);
    });
