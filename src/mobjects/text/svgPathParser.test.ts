...
describe("parseSVGPathData", () => {
  ...
  describe("path to Bezier conversion", () => {
    ...
    it("multiple sub-paths with multiple M commands", () => {
      const result = parseSVGPathData(
        "M 0 0 L 1 0 M 10 10 L 11 10 M 20 20 L 21 20",
      );
  ...
  describe("edge cases", () => {
    ...
    it("S command chain accumulates reflections", () => {
      const result = parseSVGPathData(
        "M 0 0 C 0 10 10 10 10 0 S 20 -10 20 0 S 30 10 30 0",
      );
      const sp = result[0];
  ...
  describe("structural properties", () => {
    ...
    it("all points are [number, number] tuples", () => {
      const result = parseSVGPathData(
        "M 0 0 C 1 2 3 4 5 6 L 7 8 Q 9 10 11 12 Z",
      );
  ...
  describe("smooth cubic (S/s) edge cases", () => {
    ...
    it("chained S commands after C", () => {
      const result = parseSVGPathData(
        "M 0 0 C 0 5 5 5 5 0 S 10 -5 10 0 S 15 5 15 0",
      );
      const sp = result[0];
      expect(sp).toHaveLength(10);
    ...
    it("multiple S segments in one command", () => {
      const result = parseSVGPathData(
        "M 0 0 C 1 2 3 4 5 0 S 8 -2 10 0 12 2 15 0",
      );
      const sp = result[0];
      expect(sp).toHaveLength(10);
      expectPointClose(sp[9], [15, 0]);
...
describe("svgToVMobjects", () => {
  ...
  describe("defs and use elements", () => {
    ...
    it("should skip <use> with no href at all", () => {
      const svg = createSVG(
        "<svg>" + '<defs><path id="nouse" d="M 0 0 L 5 5 Z"/></defs>' +
          "<use/>" + "</svg>",
      );
      const group = svgToVMobjects(svg);
      expect(group.submobjects.length).toBe(0);
    ...
    it("should skip <defs> paths without id", () => {
      const svg = createSVG(
        "<svg>" + '<defs><path d="M 0 0 L 5 5 Z"/></defs>' + "</svg>",
      );
      const group = svgToVMobjects(svg);
      expect(group.submobjects.length).toBe(0);
    });
    ...
    it("should skip <defs> paths without d", () => {
      const svg = createSVG(
        "<svg>" + '<defs><path id="nod"/></defs>' + '<use href="#nod"/>' +
          "</svg>",
      );
      const group = svgToVMobjects(svg);
      expect(group.submobjects.length).toBe(0);
  ...
  describe("nested <g> transforms", () => {
    ...
    it("should handle <g> without transform", () => {
      const svg = createSVG(
        "<svg>" + "<g>" + '<path d="M 0 0 L 10 0 L 10 10 Z"/>' + "</g>" +
          "</svg>",
      );
      const group = svgToVMobjects(svg);
      expect(group.submobjects.length).toBe(1);
  ...
  describe("rect elements", () => {
    it("should convert <rect> to VMobject", () => {
      const svg = createSVG(
        '<svg><rect x="10" y="20" width="100" height="50"/></svg>',
      );
      const group = svgToVMobjects(svg);
      expect(group.submobjects.length).toBe(1);
    });
    ...
    it("should skip <rect> with zero width", () => {
      const svg = createSVG(
        '<svg><rect x="0" y="0" width="0" height="50"/></svg>',
      );
      const group = svgToVMobjects(svg);
      expect(group.submobjects.length).toBe(0);
    });
    ...
    it("should skip <rect> with zero height", () => {
      const svg = createSVG(
        '<svg><rect x="0" y="0" width="50" height="0"/></svg>',
      );
      const group = svgToVMobjects(svg);
      expect(group.submobjects.length).toBe(0);
    });
  ...
  describe("scale() transforms on <g> elements", () => {
    it("should scale path coordinates by <g> scale factor", () => {
      ...
      const svg = createSVG(
        "<svg>" + '<g transform="scale(0.5)">' + '<path d="M 0 0 L 100 0"/>' +
          "</g>" + "</svg>",
      );
      ...
      const svgNoScale = createSVG(
        "<svg>" + '<path d="M 0 0 L 100 0"/>' + "</svg>",
      );
      const groupUnscaled = svgToVMobjects(svgNoScale, {
        flipY: false,
        scale: 1,
      });

      const ptsScaled = (groupScaled.submobjects[0] as any).getLocalPoints();
      const ptsUnscaled = (groupUnscaled.submobjects[0] as any)
        .getLocalPoints();
    ...
    it("should handle translate(a,b) scale(s) combined", () => {
      ...
      const grpNoScale = svgToVMobjects(svgTranslateOnly, {
        flipY: false,
        scale: 1,
      });
      ...
      expect(endScale[endScale.length - 1][0]).toBeLessThan(
        endNoScale[endNoScale.length - 1][0],
      );
    });
    ...
    it("should apply scale to <use> glyph references", () => {
      ...
      const svgNoScale = createSVG(
        "<svg>" + '<defs><path id="g1" d="M 0 0 L 100 0"/></defs>' +
          '<use href="#g1"/>' + "</svg>",
      );
    ...
    it("should apply scale to <use> x/y offsets", () => {
      ...
      const startScale =
        (grpScale.submobjects[0] as any).getLocalPoints()[0][0];
      const startNoScale =
        (grpNoScale.submobjects[0] as any).getLocalPoints()[0][0];
