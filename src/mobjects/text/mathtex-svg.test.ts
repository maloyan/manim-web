...
describe("Issue #228: MathTex defaults to SVG mode", () => {
  ...
  it("MathTex should have SVG-specific options (strokeWidth, fillOpacity)", () => {
    ...
    expect((tex as unknown as { _svgStrokeWidth: number })._svgStrokeWidth)
      .toBe(3);
    expect((tex as unknown as { _svgFillOpacity: number })._svgFillOpacity)
      .toBe(0.8);
  });
  ...
  it("MathTex multi-part should be a VGroup with isMultiPart flag", () => {
    ...
    expect((tex as unknown as { _isMultiPart: boolean })._isMultiPart).toBe(
      true,
    );
  });
