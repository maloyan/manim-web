...
describe("Paragraph", () => {
  ...
  describe("_measureText override", () => {
    ...
    it("should store wrappedLines after measurement", () => {
      ...
      expect(Array.isArray((p as unknown as ParagraphInternal)._wrappedLines))
        .toBe(true);
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("should not throw for zero width", () => {
      expect(() => new Paragraph({ text: "Zero width", width: 0 })).not
        .toThrow();
    });
  ...
  describe("_isNewParagraph", () => {
    it("should return true for last line", () => {
      ...
      const lines = ["Single"];
      expect((p as unknown as ParagraphInternal)._isNewParagraph(lines, 0))
        .toBe(true);
    });
    ...
    it("should return true when index is beyond bounds", () => {
      const p = new Paragraph({ text: "Single" });
      const lines = ["Single"];
      expect((p as unknown as ParagraphInternal)._isNewParagraph(lines, 5))
        .toBe(true);
    });
  });
    ...
    it("should render with justify alignment without error", () => {
      expect(
        () =>
          new Paragraph({
            text:
              "This is justified text that should wrap across multiple lines when constrained",
            alignment: "justify",
            width: 0.5,
          }),
      ).not.toThrow();
  ...
  describe("_measureText width constraint", () => {
    it("should constrain measured width to maxWidth when set", () => {
      const p = new Paragraph({
        text:
          "A very long line that exceeds the maximum width constraint set by the user",
        width: 2,
      });
