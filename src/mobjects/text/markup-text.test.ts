...
describe("MarkupText", () => {
  ...
  describe("basic Pango tags", () => {
    ...
    it("should parse <strikethrough> as strikethrough", () => {
      expect(segs("<strikethrough>S</strikethrough>")[0].strikethrough).toBe(
        true,
      );
    });
  });
  ...
  describe("<span> attributes", () => {
    ...
    it("should parse font_family attribute", () => {
      expect(segs('<span font_family="Courier">C</span>')[0].fontFamily).toBe(
        "Courier",
      );
    });

    it("should parse font attribute as font_family", () => {
      expect(segs('<span font="Helvetica">H</span>')[0].fontFamily).toBe(
        "Helvetica",
      );
    });

    it("should parse face attribute as font_family", () => {
      expect(segs('<span face="Verdana">V</span>')[0].fontFamily).toBe(
        "Verdana",
      );
    });
    ...
    it('should parse weight="normal"', () => {
      expect(segs('<span weight="normal">N</span>')[0].fontWeight).toBe(
        "normal",
      );
    });
    ...
    it('should parse style="oblique"', () => {
      expect(segs('<span style="oblique">O</span>')[0].fontStyle).toBe(
        "oblique",
      );
    });
    ...
    it('should parse strikethrough="true"', () => {
      expect(segs('<span strikethrough="true">S</span>')[0].strikethrough).toBe(
        true,
      );
    });

    it('should parse strikethrough="false"', () => {
      expect(segs('<span strikethrough="false">N</span>')[0].strikethrough)
        .toBe(false);
    });

    it("should parse background attribute", () => {
      expect(segs('<span background="yellow">H</span>')[0].backgroundColor)
        .toBe("yellow");
    });

    it("should parse bgcolor attribute", () => {
      expect(segs('<span bgcolor="#eee">B</span>')[0].backgroundColor).toBe(
        "#eee",
      );
    });

    it('should parse variant="small-caps"', () => {
      expect(segs('<span variant="small-caps">C</span>')[0].fontVariant).toBe(
        "small-caps",
      );
    });
  ...
  describe("special tags", () => {
    ...
    it("should parse <tt> as monospace font", () => {
      expect(
        new MarkupText({ text: "<tt>c</tt>" }).getStyledSegments()[0]
          .fontFamily,
      ).toBe(
        "monospace",
      );
  ...
  describe("getStyledSegments", () => {
    it("should return readonly array", () => {
      expect(
        Array.isArray(new MarkupText({ text: "<b>B</b>" }).getStyledSegments()),
      ).toBe(true);
    });
  ...
  describe("codeFontFamily", () => {
    it("should default to monospace", () => {
      expect(new MarkupText({ text: "T" }).getCodeFontFamily()).toBe(
        "monospace",
      );
    });
  ...
  describe("edge cases", () => {
    ...
    it("should handle font_size percentage", () => {
      expect(segs('<span font_size="120%">X</span>')[0].fontSize).toBeCloseTo(
        57.6,
        0,
      );
    });
    ...
    it('should handle font_size "larger"', () => {
      expect(segs('<span font_size="larger">X</span>')[0].fontSize).toBeCloseTo(
        57.6,
        0,
      );
    });
    ...
    it('should handle font_size "smaller"', () => {
      expect(segs('<span font_size="smaller">X</span>')[0].fontSize)
        .toBeCloseTo(39.84, 0);
    });

    it("should handle named absolute sizes", () => {
      // Default fontSize is 48. Named sizes are multipliers.
      expect(segs('<span font_size="xx-small">X</span>')[0].fontSize)
        .toBeCloseTo(48 * 0.5789, 0);
      expect(segs('<span font_size="medium">X</span>')[0].fontSize).toBeCloseTo(
        48,
        0,
      );
      expect(segs('<span font_size="x-large">X</span>')[0].fontSize)
        .toBeCloseTo(48 * 1.44, 0);
      expect(segs('<span font_size="xx-large">X</span>')[0].fontSize)
        .toBeCloseTo(48 * 1.728, 0);
    });

    it("should handle Pango units (>200) in font_size", () => {
      // Value > 200 => divide by 1024
      expect(segs('<span font_size="24576">X</span>')[0].fontSize).toBeCloseTo(
        24,
        0,
      );
    });
    ...
    it("should return current size for unparseable font_size", () => {
      expect(segs('<span font_size="abc">X</span>')[0].fontSize).toBeCloseTo(
        48,
        0,
      );
    });
  ...
  describe("rendering integration", () => {
    it("should construct with different textAlign options", () => {
      ...
      const right = new MarkupText({
        text: "<b>Right</b>",
        textAlign: "right",
      });
      const center = new MarkupText({
        text: "<b>Center</b>",
        textAlign: "center",
      });
    ...
    it("should handle background color segment", () => {
      const mt = new MarkupText({
        text: '<span background="yellow">Highlighted</span>',
      });
