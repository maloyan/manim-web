...
describe("MathTex (SVG)", () => {
  ...
  const getRenderedHeightFromPoints = (
    svg: { children: unknown[] },
  ): number => {
  ...
  describe("getPart", () => {
    it("should throw on single-part MathTex", () => {
      const svg = new MathTex({ latex: "x" });
      expect(() => svg.getPart(0)).toThrow(
        "getPart() is only available on multi-part MathTex",
      );
    });
  ...
  describe("_scaleToTarget", () => {
    ...
    it("should render x near 0.45em at default 48pt", async () => {
      ...
      expect(measuredHeight).toBeCloseTo(
        0.45 * DEFAULT_FONT_SIZE_IN_WORLD_SPACE,
        2,
      );
    });
...
describe("GlyphVMobject", () => {
  ...
  describe("constructor", () => {
    ...
    it("should accept xOffset and yOffset", () => {
      const commands = [{ type: "M", x: 0, y: 0 }, { type: "L", x: 10, y: 0 }, {
        type: "Z",
      }];
      const glyph = createMockGlyph(commands);
      const font = createMockFont();
...
describe("MathJaxRenderer", () => {
  ...
  describe("renderLatexToSVG", () => {
    ...
    it("ignores legacy fontScale option in renderer options", async () => {
      const base = await renderLatexToSVG(
        "x",
        { displayMode: true, color: "#ffffff" } as any,
      );
      ...
      const getHeight = (
        res: { vmobjectGroup: { children: unknown[] } },
      ): number => {
        let minY = Number.POSITIVE_INFINITY;
        let maxY = Number.NEGATIVE_INFINITY;
