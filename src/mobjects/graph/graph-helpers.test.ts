...
describe("Layout algorithms", () => {
  ...
  it("tree layout defaults to first vertex as root", () => {
    ...
    expect(g.getPositions().get(1)![1]).toBeGreaterThan(
      g.getPositions().get(2)![1],
    );
  });
  ...
  it("layout string shorthand sets type", () => {
    expect(
      new Graph({ vertices: [1, 2, 3], layout: "circular" }).getPositions()
        .size,
    ).toBe(3);
  });
  ...
  it("setLayout returns this for chaining", () => {
    expect(new Graph({ vertices: [1] }).setLayout("circular")).toBeInstanceOf(
      Graph,
    );
  });
...
describe("Styling operations", () => {
  it("setVertexColor returns this for chaining", () => {
    expect(new Graph({ vertices: [1] }).setVertexColor(1, "#FF0000"))
  ...
  it("setEdgeColor returns this for chaining", () => {
    expect(
      new Graph({ vertices: [1, 2], edges: [[1, 2]] }).setEdgeColor(
        1,
        2,
        "#0F0",
      ),
