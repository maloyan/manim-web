...
export class ShapeMorphStrategy implements MorphStrategy {
  ...
  private _getVisualDims(
    ...
    if (!(geometry instanceof THREE.PlaneGeometry)) {
      throw new Error(
        `ShapeMorphStrategy requires ${label} geometry to be THREE.PlaneGeometry`,
      );
    }
    const params = geometry.parameters;
    if (typeof params.width !== "number" || typeof params.height !== "number") {
      throw new Error(
        `ShapeMorphStrategy requires ${label} plane geometry width/height`,
      );
    }
    return {
  ...
  private _initTexturedMeshes(source: Mobject, target: Mobject): void {
    if (!(source instanceof TexturedMobject)) {
      throw new Error(
        "ShapeMorphStrategy.begin requires source TexturedMobject",
      );
    }
    if (!(target instanceof TexturedMobject)) {
      throw new Error(
        "ShapeMorphStrategy.begin requires target TexturedMobject",
      );
    }
  ...
  begin(_animation: Animation, source: Mobject, target: Mobject): void {
    ...
    if (!this._shapeSourceMesh) {
      throw new Error("ShapeMorphStrategy requires _shapeSourceMesh");
    }
    if (!this._shapeTargetMesh) {
      throw new Error("ShapeMorphStrategy requires _shapeTargetMesh");
    }
  ...
  interpolate(
    ...
    const h = this._startHeight +
      (this._targetHeight - this._startHeight) * alpha;
    this._shapeSourceMesh.scale.set(w, h, 1);
    this._shapeTargetMesh.scale.set(w, h, 1);
    source.position.lerpVectors(
      this._startPosition,
  ...
  finish(_animation: Animation, source: Mobject, _target: Mobject): void {
    ...
    this._sourceTextureMobject.applyVisualSize(
      this._targetWidth,
      this._targetHeight,
    );
