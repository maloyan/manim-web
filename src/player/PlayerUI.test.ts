...
function createMockTimeline(
  slides: Array<{ index: number; startTime: number }>,
  duration: number,
) {
  return {
    getSegments: () => slides,
...
describe("PlayerUI", () => {
  ...
  afterEach(() => {
    ...
    document.body.querySelectorAll(":scope > [data-player-bar]").forEach((el) =>
      el.remove()
    );
    vi.useRealTimers();
  });
  ...
  describe("export menu", () => {
    ...
    it("toggles export menu closed on second click", () => {
      ...
      // Open
      exportBtn.click();
      let bodyMenus = document.body.querySelectorAll(
        ":scope > [data-player-bar]",
      );
  ...
  describe("export menu item hover", () => {
    it("highlights menu item on mouseenter", () => {
      ...
      const allBars = document.body.querySelectorAll(
        ":scope > [data-player-bar]",
      );
      const menu = allBars[allBars.length - 1];
