...
import { MovingCamera, MultiCamera, ThreeDCamera } from "./CameraExtensions";
...
describe("CameraAnimateProxy", () => {
  ...
  it("chaining multiple operations", () => {
    const proxy = frame.animate.incrementTheta(1).setDistance(20)
      .setFieldOfView(60);
    proxy.begin();
    proxy.interpolate(1);
    expect(frame.getTheta()).toBeCloseTo(1);
...
describe("MultiCamera", () => {
  ...
  describe("render()", () => {
    ...
    it("sets correct viewport pixel coordinates from fractional viewports", () => {
      ...
      expect(renderer.setViewport).toHaveBeenNthCalledWith(
        1,
        250,
        50,
        500,
